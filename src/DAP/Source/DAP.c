...
void Delayms(uint32_t delay) {
    delay *= ((CPU_CLOCK / 1000U) + (DELAY_SLOW_CYCLES - 1U))
            / DELAY_SLOW_CYCLES;
...
static uint32_t DAP_Delay(const uint8_t *request, uint8_t *response) {
    ...
    delay *= ((CPU_CLOCK / 1000000U) + (DELAY_SLOW_CYCLES - 1U))
            / DELAY_SLOW_CYCLES;
...
static uint32_t DAP_SWD_Transfer(const uint8_t *request, uint8_t *response) {
        ...
        if ((request_value & DAP_TRANSFER_RnW) != 0U) {
            ...
            if (post_read) {
                ...
                if ((request_value
                    ...
                    do {
                        response_value = SWD_Transfer(
                                DP_RDBUFF | DAP_TRANSFER_RnW, &data);
                    } while ((response_value == DAP_TRANSFER_WAIT) && retry--
                            && !DAP_TransferAbort);
                    post_read = 0U;
                }
            ...
            if ((request_value & DAP_TRANSFER_MATCH_VALUE) != 0U) {
                ...
                } while (((data & DAP_Data.transfer.match_mask) != match_value)
            ...
            if (post_read) {
                ...
                retry = DAP_Data.transfer.retry_count;
                do {
                    response_value = SWD_Transfer(DP_RDBUFF | DAP_TRANSFER_RnW,
                            &data);
                } while ((response_value == DAP_TRANSFER_WAIT) && retry--
                        && !DAP_TransferAbort);
                if (response_value != DAP_TRANSFER_OK) {
    ...
    end: *(response_head + 0) = (uint8_t) response_count;
    ...
    for (; request_count != 0U; request_count--) {
        ...
        if ((request_value & DAP_TRANSFER_RnW) != 0U) {
            ...
            if (post_read) {
                ...
                retry = DAP_Data.transfer.retry_count;
                do {
                    response_value = JTAG_Transfer(DP_RDBUFF | DAP_TRANSFER_RnW,
                            &data);
                } while ((response_value == DAP_TRANSFER_WAIT) && retry--
                        && !DAP_TransferAbort);
                if (response_value != DAP_TRANSFER_OK) {
...
static uint32_t DAP_SWD_TransferBlock(const uint8_t *request, uint8_t *response) {
    ...
    end: *(response_head + 0) = (uint8_t) (response_count >> 0);
...
//   return:   number of bytes in response (lower 16 bits)
//             number of bytes in request (upper 16 bits)
__WEAK uint32_t DAP_ProcessVendorCommand(const uint8_t *request,
        uint8_t *response) {
    (void) request;
